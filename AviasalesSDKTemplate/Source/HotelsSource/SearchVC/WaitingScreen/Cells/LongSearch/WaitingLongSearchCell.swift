...
class WaitingLongSearchCell: HLAutolayoutCollectionViewCell {
    ...
    class func preferredHeight(containerWidth: CGFloat) -> CGFloat {
        ...
        let titleHeight = titleText().hl_height(attributes: [NSAttributedString.Key.font: titleFont()], width: maxTextWidth)
        let subtitleHeight = subtitleText().hl_height(attributes: [NSAttributedString.Key.font: subtitleFont()], width: maxTextWidth)
